...
/// <summary>
/// VeryHighPointを獲得した際に表示されるポイントのプールを管理するクラス。UI要素の再利用を行い、パフォーマンスを向上させる。
/// ゲーム中のポイント表示計UIはベースPoolクラスを作成したほうが良いと思うからこれはTODO
/// </summary>
public class VeryHighPointPool : MonoBehaviour, IPlayUIReturner
{
    ...
    private PooledPlayUIItem CreateNew(Transform parent)
    {
        if (prefab == null)
        {
            Debug.LogError($"{nameof(NewOrbitPointPool)}: prefab が未設定です。", this);
            return null;
        }
